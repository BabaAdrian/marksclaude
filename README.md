...
- **Backend database.** All data (classes, units, students, marks, mock scores, teacher
  ...
  See **Deploying to Vercel + Supabase** below for exact steps.
...
- **Live updates.** Changes made by one person show up for everyone else automatically —
...
While signed in, the app quietly checks for changes every few seconds — so if a teacher
enters marks in one browser, an admin looking at reports in another sees them without
refreshing. A small "Live" indicator in the top bar shows this is active.

A live check never lands while you're actively typing (it waits until you've paused for
a few seconds), so it can't yank back something mid-edit. Saves are also debounced —
typing fires an instant on-screen update, but the actual save to the database only goes
out shortly after you pause, and never more than one save is in flight at a time.

Two people editing *different* things around the same time — one person renaming a
class while another enters marks for a different unit — merge correctly rather than one
overwriting the other; each save only touches the specific records or fields that
browser actually changed; server changes to everything else in the meantime are left
untouched. What isn't handled — and isn't really something any app can fully solve — is
two people editing the exact same field within the same few-second window; whichever
save lands last wins there, same as most collaborative tools.
